...
//TODO falta arreglar este controlador para que mande el JSON
app.controller('JsonCtrl', ['$scope','$http', function($scope, $http) {
    ...
    function sendData(data, $http) {
      ...
      $http(req).success(function(response) {
        ...
        if(state){
          $scope.alerts = [        
          { type: 'success', msg: '¡Anuncio publicado! :)' }
              ];          
        }else{
        $scope.alerts = [        
          { type: 'danger', msg: 'No se pudo publicar la promoción :( ' + state.errors}
              ];            
        }
...
app.controller('EventArgumentsCtrl', ['$scope','$http','SharedVars', function($scope, $http, SharedVars) {
  ...
  //Esta función pone los marcadores en el mapa
    ...
    function getData($http, SharedVars) {
      ...
      $http(req).success(function(response) {
        ...
        console.log("otra cosa");
        $scope.alerts = [        
        { type: 'danger', msg: 'No cargó las sucursales' }
            ];
      });
    } 
